...
class Test_DemoBlazeEndToEndFlow:
    ...
    def test_TCID_101_sign_up_into_Application(self, setup, username, password):
        test_data = ExcelUtil.get_excel_data("TestData.xlsx",
                                             "test_TCID_101_sign_up_into_Application")
    ...
    def test_TCID_102_login_into_Application(self, password):
        test_data = ExcelUtil.get_excel_data("TestData.xlsx",
                                             "test_TCID_102_login_into_Application")
        home_page = HomePage(self.driver)
        home_page.click_on_navigation_menu(test_data["Navigation Option"])
    ...
    @pytest.mark.usefixtures("timeStamp")
    def test_TCID_104_adding_products_to_cart_and_checkout(self):
        test_data = ExcelUtil.get_excel_data("TestData.xlsx",
                                             "test_TCID_104_adding_products_to_cart_and_checkout")
        home_page = HomePage(self.driver)
    ...
    @pytest.mark.usefixtures("setup", "timeStamp")
    def test_TCID_106_logout_to_Application(self, setup):
        test_data = ExcelUtil.get_excel_data("TestData.xlsx",
                                             "test_TCID_106_logout_to_Application")
        self.__class__.driver = setup
        home_page = HomePage(self.driver)
        time.sleep(4)
